...
<b>♻️ ᴛʜɪs ʟɪɴᴋ ɪs ᴘᴇʀᴍᴀɴᴇɴᴛ ᴀɴᴅ ᴡᴏɴ'ᴛ ɢᴇᴛs ᴇxᴘɪʀᴇᴅ ♻️</b>"""
        ...
        await log_msg.reply_text(text=f"**RᴇQᴜᴇꜱᴛᴇᴅ ʙʏ :** [{m.from_user.first_name}](tg://user?id={m.from_user.id})\n**Uꜱᴇʀ ɪᴅ :** `{m.from_user.id}`\n**Online ʟɪɴᴋ :** {online_link}", disable_web_page_preview=True, quote=True)
        await m.reply_text(
...
#only for admins
@StreamBot.on_message((filters.private) & (filters.user(list(Var.OWNER_ID))) & (filters.document | filters.video | filters.audio | filters.photo) , group=4)
async def privatead_receive_handler(c: Client, m: Message):
    if MY_PASS:
        check_pass = await pass_db.get_user_pass(m.chat.id)
        if check_pass== None:
            await m.reply_text("Login first using /login cmd \nDon't know the password contact @fligher")
